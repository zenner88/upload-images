<?php
  session_start();

  $file_name =  $_FILES['file']['name']; //getting file name
  $tmp_name = $_FILES['file']['tmp_name']; //getting temp_name of file
  $file_up_name = time().$file_name; //making file name dynamic by adding time before file name
  move_uploaded_file($tmp_name, "../../../../../opt/luxand/haystack".$file_up_name); //moving file to the specified folder with dynamic name
  
  $_SESSION["foto"] = $file_up_name;

  $command = escapeshellcmd('/opt/luxand/brimob_demo.py');
  $output = shell_exec($command);
  echo $output;
?>
